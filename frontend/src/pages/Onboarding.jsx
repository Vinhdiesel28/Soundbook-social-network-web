...
const Onboarding = () => {
  ...
  const copy = useMemo(() => {
    if (language === 'vi') {
      return {
        connected: 'Spotify đã được kết nối. Bạn có thể tiếp tục vào app.',
        connectError: 'Không thể bắt đầu kết nối Spotify.',
        statusError: 'Không thể kiểm tra trạng thái Spotify.',
        continue: 'Tiếp tục vào bảng tin',
      };
    }

    return {
      connected: 'Spotify is connected. You can continue to the app.',
      connectError: 'Cannot start Spotify connection.',
      statusError: 'Cannot load Spotify status.',
      continue: 'Continue to the feed',
    };
  }, [language]);
  ...
  const fetchSpotifyStatus = async () => {
    try {
      ...
      if (localStorage.getItem('token')) {
        setSpotifyError(error.message || copy.statusError);
      }
    }
  };

  useEffect(() => {
    if (step === 3 && localStorage.getItem('token')) {
      fetchSpotifyStatus();
    }
  }, [step]);
  ...
  useEffect(() => {
    ...
    if (spotify === 'connected') {
      setSpotifyNotice(message || copy.connected);
      setSpotifyError('');
      fetchSpotifyStatus();
    } else {
      setSpotifyError(message || copy.connectError);
    }

    navigate(location.pathname, { replace: true });
  }, [location.search, location.pathname, navigate, copy.connected, copy.connectError]);
  ...
  const handleConnectSpotify = async () => {
    ...
    try {
      await startSpotifyConnect('/onboarding');
    } catch (error) {
      setSpotifyError(error.message || copy.connectError);
            ...
            {step === 3 && (
              <motion.div key="step3" variants={containerVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col h-full">
                ...
                <div className="flex-1 flex flex-col items-center justify-center min-h-[250px] space-y-4">
                  {spotifyNotice && (
                    <div className="w-full max-w-sm rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700 dark:border-green-900/60 dark:bg-green-900/20 dark:text-green-300">
                      {spotifyNotice}
                    </div>
                  )}

                  {spotifyError && (
                    <div className="w-full max-w-sm rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/60 dark:bg-red-900/20 dark:text-red-300">
                      {spotifyError}
                    </div>
                  )}

                  {spotifyStatus.connected && (
                    <div className="w-full max-w-sm rounded-xl border border-[#1DB954]/30 bg-[#1DB954]/10 px-4 py-3 text-sm text-[#1DB954]">
                      Spotify: {spotifyStatus.spotifyDisplayName || spotifyStatus.spotifyUserId || 'Connected'}
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="w-full max-w-sm flex items-center justify-center gap-3 px-6 py-4 bg-[#1DB954] text-white rounded-xl font-bold hover:bg-[#1ed760] transition-colors shadow-lg shadow-green-500/30 disabled:opacity-60"
                  >
                    ...
                    {spotifyBusy ? '...' : t('onboarding.connect_spotify')}
                  </button>
                  <button
                    onClick={() => navigate('/feed')}
                    className="w-full max-w-sm px-6 py-4 text-text-muted hover:text-text-color hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors font-medium"
                  >
                    {spotifyStatus.connected ? copy.continue : t('onboarding.skip')}
                  </button>
                </div>
